...
## Про разработку проекта

Проект разрабатывался в команде из двух человек (смотреть "Авторы"). Цель команды была написать бэкенд проекта (на Django) и API для него (на DRF). 
Я написал всю часть, касающуюся управления пользователями: систему регистрации и аутентификации, права доступа, работу с токеном (JWTAuthentication), систему подтверждения через e-mail. Так же я реализовал импортирование данных из CSV файлов в базу данных.
